...
function ProductDetail({
    ...
    const addToCart = (id) => {
        if (!user.expired) {
            if (color.length === 0) {
                ...
                api.post("/api/cart/add", post_data)
                    .then((response) => {
    ...
    useEffect(() => {
        if (product.images && product.images.length > 0) {
            // Display the first image from the product's images on page load
            setBigImage(product.images[0]);
        }
    }, [product]);

    const smallImageClickHandler = (imageSource, index) => {
    ...
    return (
        <>
            ...
            <div className='container flex flex-col md:grid md:grid-cols-2 gap-6 '>
                <div>
                    <img
                        src={`https://tnz-creations-images.s3.ap-south-1.amazonaws.com/${bigImage}`}
                        alt='product'
                    ...
                    <div className='grid grid-cols-5 gap-4 mt-4'>
                        {product &&
                            ...
                            product.images.map((src, index) => (
                                <img
                                    key={index}
                                    src={`https://tnz-creations-images.s3.ap-south-1.amazonaws.com/${src}`}
                                    alt='product'
                                    className={`w-full aspect-square cursor-pointer rounded-sm ${
                                        index === activeIndex
                                            ? "drop-shadow-lg border-2 border-primary"
                                            : ""
                                    }`}
                                    loading='lazy'
                                    onClick={() =>
                                        smallImageClickHandler(src, index)
                                    }
                                />
                            ))}
                    </div>
